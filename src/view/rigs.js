...
const createRigTemplate = (rig, index) => {
...
export const createRigsTemplate = (rigs) => {
  const rigsArr = []
  
  rigs.map((rig, i) => {
    rigsArr.push(createRigTemplate(rig, i + 1))
  })
  
  return `<section class="rigs">${rigsArr.join('')}</section>`
}
...
export default class Rigs {
  ...
  getTemplate() {
    return createRigsTemplate(this._rigs)
  }
